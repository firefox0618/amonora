...
> **Примечание:** Дания поддерживает **только VLESS**. Trojan не поддерживается на этой ноде.
...
`bot/device_limit_hardening.py` — принудительное применение лимитов устройств на VPN-нодах:

- Проверяет каждый VpnClient на соответствие лимитам IP
- Enforcement на нодах (3x-ui API, Xray core SSH)
- Обновляет metadata устройства (country_code, provider_type)
- Возвращает `DeviceLimitEnforcementResult` со статусом каждой операции
- Вызывается ops-скриптом, не из основного flow бота
