...
**`main_router`** из `bot/router.py` (~4900 строк) подключён **первым** и перехватывает все пользовательские взаимодействия. Legacy-обработчики из `bot/handlers/` **недостижимы** для обычных пользователей, но сохраняются как shared-логика для dashboard.
...
```
┌─────────────────────────────────────────────────────────┐
│ [Открыть подписку URL]    [Открыть в Happ URL]          │
│ [Скопировать ссылку]                                     │
│ [Мои устройства]                                         │
│ [Назад]                                                  │
└─────────────────────────────────────────────────────────┘
...
|---------|--------|------------|
| `start.py` | 🟡 Перехвачен | `/start` и `/menu` обрабатываются v2_router |
...
|------|----------|
| `main.py` | Точка входа |
| `router.py` | **V2-интерфейс** — подключён в основной бот |
...
graph TB
    subgraph "Telegram"
        Bot["@amonora_bot<br/>v2_router (test_bot) + legacy"]
