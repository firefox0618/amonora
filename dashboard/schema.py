...
from backend.core.schema import SCHEMA_UPDATES
...
async def ensure_dashboard_schema() -> None:
    async with engine.begin() as connection:
        ...
        await connection.execute(text("ALTER TABLE dashboard_admins ADD COLUMN IF NOT EXISTS avatar_path VARCHAR(255) NULL"))
        
        # Marketing campaign analytics tables
        ...
        """))
        await connection.execute(text("CREATE INDEX IF NOT EXISTS ix_marketing_campaigns_token ON marketing_campaigns(token)"))
        ...
        """))
        await connection.execute(text("CREATE INDEX IF NOT EXISTS ix_campaign_events_token ON campaign_events(campaign_token)"))
        await connection.execute(text("CREATE INDEX IF NOT EXISTS ix_campaign_events_name ON campaign_events(event_name)"))
        ...
        """))
        await connection.execute(text("CREATE INDEX IF NOT EXISTS ix_task_comments_task ON task_comments(task_id)"))
        
        # New task board columns
        await connection.execute(text("ALTER TABLE task_board ADD COLUMN IF NOT EXISTS priority VARCHAR(20) DEFAULT 'medium' NOT NULL"))
        await connection.execute(text("ALTER TABLE task_board ADD COLUMN IF NOT EXISTS color VARCHAR(10) DEFAULT '#3b82f6' NOT NULL"))
        await connection.execute(text("ALTER TABLE task_board ADD COLUMN IF NOT EXISTS assignee VARCHAR(200) NULL"))
        await connection.execute(text("ALTER TABLE task_board ADD COLUMN IF NOT EXISTS due_date TIMESTAMP NULL"))
        await connection.execute(text("ALTER TABLE task_board ADD COLUMN IF NOT EXISTS tags TEXT NULL"))
        await connection.execute(text("ALTER TABLE task_board ADD COLUMN IF NOT EXISTS checklist TEXT NULL"))
